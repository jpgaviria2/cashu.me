...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = me.cashu.wallet.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                me.cashu.wallet.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = me.cashu.wallet.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                me.cashu.wallet.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = me.cashu.wallet.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = me.cashu.wallet.util.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = me.cashu.wallet.util.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                me.cashu.wallet.model.NoisePayloadType.DELIVERED -> {
                ...
                me.cashu.wallet.model.NoisePayloadType.READ_RECEIPT -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = me.cashu.wallet.model.NoisePayload(
                type = me.cashu.wallet.model.NoisePayloadType.DELIVERED,
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                me.cashu.wallet.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    me.cashu.wallet.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    me.cashu.wallet.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }

                // Determine iOS-style guidance text
                val rel = me.cashu.wallet.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val sys = me.cashu.wallet.model.BitchatMessage(
                    sender = "system",
