package me.cashu.wallet.mesh

import android.content.Context
import android.util.Log
import me.cashu.wallet.crypto.EncryptionService
import me.cashu.wallet.model.BitchatMessage
import me.cashu.wallet.protocol.MessagePadding
import me.cashu.wallet.model.RoutedPacket
import me.cashu.wallet.model.IdentityAnnouncement
import me.cashu.wallet.protocol.BitchatPacket
import me.cashu.wallet.protocol.MessageType
import me.cashu.wallet.protocol.SpecialRecipients
import me.cashu.wallet.model.RequestSyncPacket
import me.cashu.wallet.sync.GossipSyncManager
import me.cashu.wallet.util.toHexString
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { me.cashu.wallet.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    me.cashu.wallet.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    me.cashu.wallet.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    me.cashu.wallet.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
    ...
    private fun setupDelegates() {
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    me.cashu.wallet.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        me.cashu.wallet.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
        ...
        connectionManager.delegate = object : BluetoothConnectionManagerDelegate {
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    me.cashu.wallet.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: me.cashu.wallet.model.BitchatFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = me.cashu.wallet.model.NoisePayload(
                            type = me.cashu.wallet.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = me.cashu.wallet.model.PrivateMessagePacket(
                    ...
                    val messagePayload = me.cashu.wallet.model.NoisePayload(
                        type = me.cashu.wallet.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(me.cashu.wallet.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }
            
            try {
                // Create read receipt payload using NoisePayloadType exactly like iOS
                val readReceiptPayload = me.cashu.wallet.model.NoisePayload(
                    type = me.cashu.wallet.model.NoisePayloadType.READ_RECEIPT,
     ...
     */
    fun getSessionState(peerID: String): me.cashu.wallet.noise.NoiseSession.NoiseSessionState {
