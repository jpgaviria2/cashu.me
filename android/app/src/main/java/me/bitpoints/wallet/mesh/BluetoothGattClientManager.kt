...
import me.bitpoints.wallet.ui.debug.DebugSettingsManager
import me.bitpoints.wallet.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    fun start(): Boolean {
        ...
        try {
            if (!me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value) {
    ...
    fun onScanStateChanged(shouldScan: Boolean) {
        val enabled = try { me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
