...
class BluetoothConnectionManager(
    ...
    init {
        ...
        try {
            val dbg = me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance()
            // Role enable/disable
    ...
    fun startServices(): Boolean {
        ...
        // }
            ...
            connectionScope.launch {
                ...
                val dbg = try { me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            val serverEnabled = try { me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true }
            if (serverEnabled) {
            ...
            if (wasUsingDutyCycle != nowUsingDutyCycle) {
                ...
                val clientEnabled = try { me.bitpoints.wallet.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
                if (clientEnabled) {
